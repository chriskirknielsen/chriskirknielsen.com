...
Both of these will technically "scope" the selector, however `@scope` has a few more advantages I'd say, namely specificity won't go out of control as much, and the selector has way better legibility (it's never good to repeat the target element in the same selector!), but still cool we can get to similar results in simple setups with relatively old browser versions!